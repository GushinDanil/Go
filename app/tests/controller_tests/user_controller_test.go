...
func TestCreateUser(t *testing.T) {
	...
	for _, v := range samples {
		...
		handler := http.HandlerFunc(appTest.CreateUser)
...
func TestGetUsers(t *testing.T) {
	...
	rr := httptest.NewRecorder()
	handler := http.HandlerFunc(appTest.GetUsers)
	handler.ServeHTTP(rr, req)
...
func TestGetUserByID(t *testing.T) {
	...
	for _, v := range userSample {
		...
		req = mux.SetURLVars(req, map[string]string{"id": v.id})//установил id как параметр адресной строки для получения элемента
		rr := httptest.NewRecorder() //хранит статус код того что получилось
		handler := http.HandlerFunc(appTest.GetUser)
		handler.ServeHTTP(rr, req)

		responseMap := make(map[string]interface{})//хранит в себе json ответ нашего запроса
		err = json.Unmarshal(rr.Body.Bytes(), &responseMap)
		if err != nil {
...
func TestUpdateUser(t *testing.T) {
	...
	token, err := appTest.SignIn(UserEmail, UserPassword)
	if err != nil {
	...
	for key, v := range samples {
		...
		rr := httptest.NewRecorder()
		handler := http.HandlerFunc(appTest.UpdateUser)
...
func TestDeleteUser(t *testing.T) {
	...
	token, err := appTest.SignIn(AuthEmail, AuthPassword)
	if err != nil {
		log.Fatalf("cannot login: %v\n", err)
	}
	tokenString := fmt.Sprintf("Bearer %v", token)
	...
	for _, v := range userSample {
		...
		req = mux.SetURLVars(req, map[string]string{"id": v.id})
		rr := httptest.NewRecorder()
		handler := http.HandlerFunc(appTest.DeleteUser)
